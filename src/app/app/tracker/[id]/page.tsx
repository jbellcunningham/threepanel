 ...
 *
 * ARCHITECTURE ROLE:
 * - Client-side UI for one tracker
...
/**
 * Extracts the tracker id from the current route pathname.
 */
...
async function loadStats() {
  try {
    ...
    const response = await fetch(`/api/tracker/${trackerId}/stats`, {
      cache: "no-store",
    });
  ...
  async function addEntry() {
    ...
    const res = await fetch(`/api/tracker/${trackerId}/entries`, {
      method: 'POST',
  ...
  async function saveEdit() {
    if (!trackerId || !editingEntryId || !canSave) return

    setSaving(true)
    setError(null)

    const res = await fetch(`/api/tracker/${trackerId}/entries/${editingEntryId}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      credentials: 'include',
      body: JSON.stringify({
  ...
  async function deleteEntry(entryId: string) {
    ...
    const res = await fetch(`/api/tracker/${trackerId}/entries/${entryId}`, {
      method: 'DELETE',
      credentials: 'include',
    })
		...
		{Object.entries(stats.fields).map(([fieldId, field]) => {
          ...
          >
            ...
            {!schema?.fields?.length ? (
              <div style={{ opacity: 0.75 }}>This tracker has no schema yet.</div>
            ) : (
